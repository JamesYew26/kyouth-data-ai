...
def call_llm_with_retry(
    file_path: str, max_retries: int = 3, base_delay: float = 2.0
) -> List[str]:
    """
    ...
    try:
        # ✅ FIXED INDENTATION: Safely indented inside the try block scope
        ...
        print(
            f"--- [find_skill_gaps.py] Extracted {len(resume_text)} text characters from disk PDF ---"
        )
    except Exception as pdf_err:
        logging.error(f"Failed to extract PDF data stream natively: {pdf_err}")
        return []
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                ...
                raise e
            logging.warning(
                f"LLM attempt {attempt + 1} failed: {e}. Retrying in {delay} seconds..."
            )
            time.sleep(delay)
...
def fetch_db_skills_batched(
    db_url: str, batch_size: int = 500, max_retries: int = 3, base_delay: float = 2.0
) -> Set[str]:
    """
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if attempt == max_retries:
                ...
                raise e
            logging.warning(
                f"Database attempt {attempt + 1} failed: {e}. Retrying in {delay} seconds..."
            )
            time.sleep(delay)
            delay *= 2
        finally:
...
def find_skill_gaps(input_file_path: str, db_url: str) -> SkillGapResult:
    ...
    try:
        ...
        logging.error(
            f"Critical error encountered in find_skill_gaps pipeline: {final_error}"
        )
